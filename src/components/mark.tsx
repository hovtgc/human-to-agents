import { cn } from "@/lib/utils";
import { REPO } from "@/lib/repo/meta";

export function OwnerAvatar({ className }: { className?: string }) {
  return (
    <img
      src={REPO.ownerAvatar}
      alt=""
      width={32}
      height={32}
      className={cn(
        "size-8 shrink-0 rounded-full object-cover outline outline-1 -outline-offset-1 outline-line",
        className,
      )}
    />
  );
}

export function GitHubMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 16 16" className={cn("fill-current", className)} aria-hidden>
      <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8" />
    </svg>
  );
}

export function XMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={cn("fill-current", className)} aria-hidden>
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.727-8.835L1.254 2.25H8.08l4.253 5.622L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

const jumpClass =
  "flex h-10 items-center gap-1.5 rounded-full px-2 text-ink hover:bg-panel active:scale-[0.96] sm:px-2.5";

export function OwnerJump() {
  return (
    <div className="flex shrink-0 items-center rounded-full border border-line bg-raised p-0.5">
      <a
        href={REPO.xUrl}
        target="_blank"
        rel="noreferrer"
        className={jumpClass}
        aria-label={`@${REPO.owner} on X`}
      >
        <OwnerAvatar className="size-7" />
        <XMark className="size-3.5" />
        <span className="text-xs font-medium">X</span>
      </a>
      <span className="h-5 w-px bg-line" aria-hidden />
      <a
        href={REPO.githubProfile}
        target="_blank"
        rel="noreferrer"
        className={jumpClass}
        aria-label="GitHub"
      >
        <GitHubMark className="size-4" />
        <span className="text-xs font-medium sm:hidden">GH</span>
        <span className="hidden text-xs font-medium sm:inline">GitHub</span>
      </a>
    </div>
  );
}

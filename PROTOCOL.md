...
- **v0.3** — optional `work` table. Each row is one broadcast, joined to a mapped room. Notion (or any PM) may *materialize* this table; the relay still only reads markdown.
...
| `anchor` | no | Platform thread id. Present → already posted, skip. |
...
3. Without a work table: fan-out to every `room` in the map, excluding the source.
   With a work table: one post per `active`/`blocked` row into that row's `room`. Skip if `anchor` is set. Gap if the room is not mapped.
...
# report from {agent}
scope: {scope}
work: {n}  post: {n}  skip: {n}  gaps: {n}

## {work.id} — {state}
room: {id}
anchor: {ts or (none — would post)}
```
